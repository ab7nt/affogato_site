  ...
  scenes: {
    ...
    songs: {
      ...
      items: [
        ...
        {
          title: 'Нам ли с тобой',
          year: '1999',
          cover: 'assets/song-covers/are-we-with-you.webp',
          audio: 'assets/songs/are-we-with-you.m4a',
          note: 'Песне тоже больше 20 лет и она почти в первозданном виде, кроме нескольких изменений в тексте для лучшего звучания.',
        },
        {
          title: 'Имя моё',
          year: '2005',
          cover: 'assets/song-covers/my-name.webp',
          audio: 'assets/songs/my-name.m4a',
          note: 'Песня написана в период жизни в общаге универа. В разгар очередного веселья я вышел с гитарой в коридор, сел на лавку и написал песню в один присест. С тех пор она живёт без изменений.',
        },
      ],
    },
    stoneVideo: {
    ...
    forest: {
      ...
      // Возврат полностью ручной; авто-доигрываются только последние кадры у
      // выхода (forest.js: applyScrub). При достижении на scroll-up последних
      // N кадров секвенции — доигрываем их сами, чтобы не зависнуть в пустом
      // лесу у выхода.
      autoExitFrames: 5,
    },
    sky: {
      ...
      ascentSec: 1.2,
      startSpeed: 0.4,
      // Сборка формы по частям (sky.js: updatePieces). Каждая часть (.sky-piece)
      // привязана к своей полосе data-band в index.html — это начало окна
      // появления [band … 1]: полностью собрана только при p=1, поэтому на
      // спуске части начинают таять сразу. Здесь — числовые ручки.
      pieces: {
        driftPx: 46,      // на сколько px часть поднимается снизу при сборке
        mobileScale: 0.85,// множитель дрейфа на мобильных
        openAt: 0.985,    // с какого прогресса форма кликабельна (собрана)
      },
      // Возврат полностью ручной; авто-доигрываются только последние кадры у
      // выхода (sky.js: applyScrub). При достижении на scroll-up последних N
      // кадров секвенции — доигрываем их сами, чтобы не зависнуть в пустом небе.
      autoExitFrames: 5,
    },
  },
