...
Affogato.Config = {
  ...
  scenes: {
    ...
    forest: {
      ...
      // Обрывки текста о проекте единым столбцом (forest.js: updateFragments).
      // Контент и data-band живут в index.html; здесь — числовые ручки
      // поведения (крутятся в DevTools на лету).
      news: {
        bandHalfWidth: 0.13,  // полуширина полосы видимости обрывка по progress
        driftPx: 1240,         // вертикальный дрейф (px на единицу |progress−band|)
        maxDriftPx: 120,      // потолок дрейфа, чтобы дальние блоки не улетали
        mobileScale: 0.85,    // множитель амплитуды дрейфа на мобильных
      },
    },
    sky: {
